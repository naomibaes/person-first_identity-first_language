...
This repository contains a complete, reproducible pipeline for analysing long-run trends in **identity-first (IF)** versus **person-first (PF)** language in printed English using **Google Books Ngrams** (1940–2019). The visualization scripts contain **News on the Web (NOW)** input data that cannot be added due to copyright restrictions which limit sharing of data. 
